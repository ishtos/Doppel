...
- [x] Phase M: レッスンコンテンツ拡充 (8本追加、全カテゴリ×全難易度カバー、シーディング改善)
...
## 7. Lesson Content
- **24レッスン** (各約250-320語) ← 16 → 24 に拡充
- **カテゴリ:** ニュース(4)、ビジネス(4)、日常会話(4)、TEDスタイル(4)、スポーツ(4)、時事ネタ(4)
- **難易度別 WPM:** 初級 100 / 中級 130 / 上級 150
- **全カテゴリで初級・中級・上級をカバー**

## 8. 本日完了したタスク (2026-05-02)
- レッスンコンテンツ拡充 (Phase M)
  - `seed_data.dart`: 8本の新規レッスンを追加 (lesson-017 〜 lesson-024)
    - ビジネス初級: "First Day at Work"
    - 日常会話中級: "Weekend Plans with Friends"
    - 日常会話上級: "Apartment Hunting"
    - TEDスタイル初級: "TED: The Magic of Reading"
    - TEDスタイル中級: "TED: Sleep and Health"
    - ニュース中級: "Technology News Update"
    - スポーツ中級: "City Marathon Report"
    - 時事ネタ初級: "Renewable Energy Revolution"
  - `main.dart`: シーディングロジック改善 — 既存ユーザーにも新規レッスンを自動配信

## 9. Handover Note for Next Run
Phase A-M まで全て完了。レッスンコンテンツを16本→24本に拡充し、全6カテゴリで初級・中級・上級の全難易度をカバー。シーディングロジックも改善し、既存ユーザーがアプリ更新時に新レッスンが自動追加されるようにした。
