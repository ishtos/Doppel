...
- [x] Phase M: 実績バッジシステム (13種の実績定義、判定ロジック、一覧画面、ホーム/進捗画面統合)
...
| Home | `/home` | 挨拶、デイリー目標進捗、お気に入りレッスン、今日のレッスン、週間統計、**実績サマリー**、改善ポイント、最近の練習 |
...
| Progress | `/progress` | スコア推移グラフ、弱点分析、統計、**実績サマリー（一覧画面へのリンク）** |
| Achievements | `/achievements` | **実績一覧（13種、カテゴリ別、ロック/アンロック状態、進捗表示）** |
...
## 8. 本日完了したタスク (2026-04-17)
- 実績バッジシステム (Phase M)
  - `lib/shared/data/achievements.dart`: 13種の実績定義（練習回数5種、連続記録4種、スコア2種、探索2種）
  - `lib/features/progress/presentation/providers/achievement_provider.dart`: 実績判定ロジック（AchievementStatus、achievementsProvider、unlockedCountProvider）
  - `lib/features/progress/presentation/screens/achievement_screen.dart`: 実績一覧画面（サマリーカード、カテゴリ別セクション、ロック/アンロック表示）
  - `lib/app/router.dart`: `/achievements` ルート追加
  - `lib/features/progress/presentation/screens/progress_screen.dart`: 実績サマリーセクション追加（進捗バー、直近のアンロック実績チップ）
  - `lib/features/home/presentation/screens/home_screen.dart`: 実績バナー追加（アンロック数、次の目標表示）

## 9. Handover Note for Next Run
Phase A-M まで全て完了。実績バッジシステムを追加済み。13種の実績が練習回数・連続記録・スコア・探索の4カテゴリで定義され、ホーム画面と進捗画面から実績一覧へアクセス可能。
