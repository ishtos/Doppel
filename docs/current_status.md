...
- [x] Phase M: 練習カレンダーヒートマップ (Progress画面にGitHub風ヒートマップカレンダー追加)
...
| Progress | `/progress` | スコア推移グラフ、**練習カレンダーヒートマップ**、弱点分析、統計 |
...
## 8. 本日完了したタスク (2026-05-19)
- 練習カレンダーヒートマップ機能 (Phase M)
  - `progress_repository.dart`: `getPracticeCalendar()` メソッド追加（過去12週間の練習日別カウント）
  - `progress_provider.dart`: `practiceCalendarProvider` 追加
  - `progress_screen.dart`: `_PracticeCalendarCard` ウィジェット追加（GitHub風ヒートマップ、曜日ラベル、凡例、練習日数カウント）

## 9. Handover Note for Next Run
Phase A-M まで全て完了。Progress画面にGitHub風の練習カレンダーヒートマップを追加済み。過去12週間の練習活動を日別に色分け表示し、練習日数をカウント表示。ダークモード対応済み。
