...
- [x] Phase M: レッスン完了トラッキング & 結果シェア機能
...
8. 追加レッスンコンテンツ拡充
9. テストカバレッジ拡充（Feedback/Lesson画面のウィジェットテスト追加）
...
| Library | `/library` | カテゴリ/難易度フィルタ、**ソート(4種)、ブックマークフィルタ、練習回数バッジ**、検索、WPMバッジ、スコアバッジ、**完了チェックアイコン** |
| Lesson | `/lesson/:id` | TTS再生、速度調整、録音/キャンセル、テキスト非表示トグル、波形アニメ、**練習完了時にレッスン状態更新** |
| Feedback | `/feedback/:id` | スコア表示、テキスト比較 (diffハイライト)、録音再生、AIコーチ (リトライ対応)、**結果シェア (クリップボードコピー)** |
...
## 8. 本日完了したタスク (2026-05-12)
- レッスン完了トラッキング & 結果シェア機能 (Phase M)
  - `lesson_provider.dart`: `LessonsNotifier.markCompleted()` メソッド追加 — レッスンの `isCompleted` を `true` に、`lastPracticedAt` を現在時刻に更新
  - `lesson_screen.dart`: `_analyzeAndNavigate()` 内で `markCompleted()` を呼び出し、練習完了後にレッスン状態を更新
  - `feedback_screen.dart`: シェアボタンにクリップボードコピー機能を実装（レッスンタイトル・総合スコア・各スコアを含むテキスト）、`services.dart` / `db_providers.dart` import 追加
  - **影響範囲:**
    - Home画面: `todayLessonProvider` が未完了レッスンを正しく推薦するようになった
    - Library画面: 「最近の練習」ソートが `lastPracticedAt` を正しく参照、完了チェックアイコンが表示されるように
    - Feedback画面: シェアボタンが機能するようになった

## 9. Handover Note for Next Run
Phase A-M まで全て完了。レッスン完了トラッキングにより、ホーム画面の「今日のレッスン」が未練習レッスンを順に推薦。ライブラリの「最近の練習」ソートも機能。フィードバック画面のシェアボタンでスコアをクリップボードにコピー可能に。
...
- テストカバレッジの拡充（Feedback / Lesson 画面のウィジェットテスト）
