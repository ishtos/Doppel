...
- [x] Phase M: テストカバレッジ強化 (Feedback/Lesson画面Widget テスト、text_diff ユニットテスト)
...
- ~~Widget tests は 22件だが、Feedback / Lesson 画面のテストが不足~~ → Phase M で追加済み (36件追加、合計58件)
- FeedbackScreen のテストで `just_audio` の `AudioPlayer` がテスト環境のネイティブプラットフォーム不在によりクリーンアップ時にハングする（テストアサーション自体は全件通過、テストランナー終了の問題のみ）
...
## 8. Test Coverage
- **score_utils_test.dart** (4件) — ScoreUtils のカラー・ラベルテスト
- **streak_logic_test.dart** (4件) — ProgressRepository のストリーク計算テスト
- **widget_test.dart** (22件) — Home/Library/Progress/Navigation の Widget テスト
- **text_diff_test.dart** (13件) — computeWordDiff / buildDiffTextSpan ユニットテスト
- **feedback_screen_test.dart** (12件) — FeedbackScreen のスコア/diff/AIコーチ/アクション表示テスト
- **lesson_screen_test.dart** (11件) — LessonScreen のTTS/録音/テキスト表示テスト

## 9. 本日完了したタスク (2026-05-20)
- テストカバレッジ強化 (Phase M)
  - `test/text_diff_test.dart`: computeWordDiff 9件 + buildDiffTextSpan 4件 = 13件のユニットテスト
  - `test/feedback_screen_test.dart`: FeedbackScreen の Widget テスト 12件
  - `test/lesson_screen_test.dart`: LessonScreen の Widget テスト 11件

## 10. Handover Note for Next Run
Phase A-M まで全て完了。テストカバレッジを22件→58件に強化（+36件）。
