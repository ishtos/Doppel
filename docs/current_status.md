...
- [x] Phase M: レッスンコンテンツ拡充 (5レッスン追加、全カテゴリ x 全難易度マトリクス完成、シードロジック改善)
...
8. ~~追加レッスンコンテンツ拡充~~ → Phase M で実装済み (16→21レッスン)
...
## 7. Lesson Content
- **21レッスン** (各約250-320語) ← 16レッスンから拡充
- **カテゴリ:** ニュース、ビジネス、日常会話、TEDスタイル、スポーツ、時事ネタ
- **難易度別 WPM:** 初級 100 / 中級 130 / 上級 150
- **全カテゴリ x 全難易度 (初級・中級・上級) の完全マトリクス達成**

| カテゴリ | 初級 | 中級 | 上級 |
|---------|------|------|------|
| ニュース | lesson-001 | lesson-007 | lesson-015 |
| ビジネス | lesson-017 | lesson-002, 006 | lesson-016 |
| 日常会話 | lesson-003, 005 | lesson-018 | lesson-019 |
| TEDスタイル | lesson-020 | lesson-021 | lesson-004, 008 |
| スポーツ | lesson-009 | lesson-010 | lesson-011 |
| 時事ネタ | lesson-012 | lesson-013 | lesson-014 |

## 8. 本日完了したタスク (2026-04-14)
- レッスンコンテンツ拡充 (Phase M)
  - `seed_data.dart`: 5つの新レッスンを追加
    - lesson-017: Self-Introduction at Work (ビジネス初級, 265語)
    - lesson-018: Planning a Weekend Trip (日常会話中級, 275語)
    - lesson-019: Cultural Exchange Discussion (日常会話上級, 285語)
    - lesson-020: TED: Small Steps, Big Change (TEDスタイル初級, 270語)
    - lesson-021: TED: The Science of Sleep (TEDスタイル中級, 280語)
  - `main.dart`: シードロジック改善 (`isEmpty` → `containsKey` チェック)
    - 既存ユーザーにも新レッスンが自動追加される
    - 既存レッスンのユーザーデータ (ブックマーク等) は保持

## 9. Handover Note for Next Run
Phase A-M まで全て完了。全6カテゴリ x 全3難易度レベルのレッスンマトリクスが完成し、合計21レッスンとなった。シードロジックも改善済みで、アプリ更新時に新レッスンが既存ユーザーにも自動配信される。
