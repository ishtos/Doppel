...
- [x] Phase M: 練習カレンダー (Progress画面に月間練習カレンダー表示)
...
| Progress | `/progress` | スコア推移グラフ、**練習カレンダー (月間ヒートマップ)**、弱点分析、統計 |
...
## 8. 本日完了したタスク (2026-04-25)
- 練習カレンダー機能 (Phase M)
  - `progress_repository.dart`: `getPracticeDays()` メソッド追加（Feedbackデータから練習日のSetを取得）
  - `progress_provider.dart`: `practiceDaysProvider` 追加
  - `progress_screen.dart`: `_PracticeCalendarSection` ウィジェット追加（月間カレンダー、月ナビゲーション、練習日ハイライト、月間サマリー）

## 9. Handover Note for Next Run
Phase A-M まで全て完了。Progress画面に月間練習カレンダーを追加済み。練習した日がハイライト表示され、月ナビゲーション（前月/次月）と月間練習日数サマリーを表示。今日の日付はボーダーで強調。未来の月には進めない制御あり。
