...
- [x] Phase M: 練習ストリークカレンダー (Progress画面に月間カレンダーヒートマップ追加)
...
| Progress | `/progress` | スコア推移グラフ、**練習ストリークカレンダー**、弱点分析、統計 |
...
## 8. 本日完了したタスク (2026-04-21)
- 練習ストリークカレンダー機能 (Phase M)
  - `day_activity.dart`: DayActivity データクラス（practiceCount, avgScore, bestScore）
  - `progress_repository.dart`: `getMonthlyActivity()` メソッド追加（月別日別アクティビティ集計）
  - `progress_provider.dart`: `selectedMonthProvider` / `monthlyActivityProvider` 追加
  - `streak_calendar.dart`: StreakCalendar ウィジェット（月間カレンダーグリッド、スコア色分けヒートマップ、月ナビゲーション、凡例、月間サマリー）
  - `progress_screen.dart`: StreakCalendar をスコア推移チャートと苦手パターンの間に配置

## 9. Handover Note for Next Run
Phase A-M まで全て完了。Progress画面に練習ストリークカレンダーを追加済み。月間カレンダーヒートマップでスコアに基づく色分け表示、月ナビゲーション、凡例、月間サマリー（練習日数・平均スコア）を提供。
